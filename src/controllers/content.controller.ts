...
import { VideoStatus, UserRole, CommentStatus } from '../types/enums';
...
export class ContentController {
  ...
  /**
   * GET /api/videos/comments/admin
   * Admin comment review queue (default: PENDING).
   */
  static async listAdminVideoComments(req: Request, res: Response) {
    try {
      const user = req.user!;
      const { status, collegeId, videoId, search, page, pageSize } = req.query;

      const profile = await UserService.getUserProfile(user.userId);
      const viewerCollegeId = profile?.adminProfile?.collegeId ?? undefined;

      const result = await ContentService.listAdminVideoComments({
        viewerRole: user.role as UserRole,
        viewerCollegeId,
  ...
  /**
   * POST /api/videos/comments/audit/batch
   * Batch audit comments (college/platform admin).
   */
  static async auditVideoCommentsBatch(req: Request, res: Response) {
    try {
      const user = req.user!;
      const { ids, pass, reason } = req.body;
      ...
      const result = await ContentService.auditVideoCommentsBatch({
        adminUserId: user.userId,
        adminRole: user.role as UserRole,
        adminCollegeId,
        ids,
